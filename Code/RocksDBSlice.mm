...
- (instancetype)initWithString:(NSString *)string
{
	self = [super init];
	if (self) {
		_slice = rocksdb::Slice([string cStringUsingEncoding: NSNonLossyASCIIStringEncoding]);
	}
	return self;
}
...
- (instancetype)initWithBytes:(char *)bytes length:(size_t)length
{
	self = [super init];
	if (self) {
		_slice = rocksdb::Slice(bytes, length);
	}
	return self;
}
...
- (instancetype)initWithBytes:(char *)bytes offset:(size_t)offset length:(size_t)length
{
	self = [super init];
	if (self) {
		_slice = rocksdb::Slice(bytes + offset, length);
	}
	return self;
}
